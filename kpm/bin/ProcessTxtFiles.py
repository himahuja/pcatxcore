...
def read_in_dicts_from_txt():
    sic_dict = json.loads(open(os.path.join("../..", "sic_dict.txt")).read().replace(" '", '"').replace("':", '":').replace("{'", '{:')replace("'}", '"}'))
    file = open(os.path.join("../../data", "sic_to_description.json"), "w")
    file.write(json.dumps(sic_dict, sort_keys = True, indent = 4))
    file.close()
    naics_dict = json.loads(open(os.path.join("../..", "naics_dict.txt")).read().replace(" '", '"').replace("':", '":').replace("{'", '{:')replace("'}", '"}'))
    file = open(os.path.join("../../data", "naics_to_description.json"), "w")
    file.write(json.dumps(naics_dict, sort_keys = True, indent = 4))
    file.close()
    sic_naics_dict = json.loads(open(os.path.join("../..", "sic_naics_dict.txt")).replace(" '", '"').replace("':", '":').replace("{'", '{:')replace("'}", '"}'))
    file = open(os.path.join("../../data", "sic_to_naics.json"), "w")
    file.write(json.dumps(sic_dict, sort_keys = True, indent = 4))
    file.close()
    naics_sic_dict = json.loads(open(os.path.join("../..", "naics_sic_dict.txt")).replace(" '", '"').replace("':", '":').replace("{'", '{:')replace("'}", '"}'))
